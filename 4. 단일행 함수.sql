...
INSERT INTO varchar_compare VALUES ('101', 'SQLD', 'SQLD  '); --FALSE
...
WHERE sn = '101'
AND char_4 = 'SQLD  ' --true  'SQLD' == 'SQLD  '
;
...
FROM dual;
--dual : 아무것도 없는 테이블 = 범위테이블(함수 호출, 연산을 해보는 테이블)
...
SELECT 
    ...
    LTRIM('HELLOhello', 'HELL')  --왼쪽에서 발견된 HE 제거 -> Ohello
FROM dual;
...
SELECT 
    ...
    ROUND(38.678, 2) AS round, -- 둘쨰자리수까지 반올림 38.68
    TRUNC(38.678, 2) AS trunc  -- 자리수 이하를 삭제 38.67
    , ABS(-20) AS abs   -- 절대값
...
SELECT SYSTIMESTAMP --조금 더 정밀한 시간을 원할 때 사용
FROM dual;
...
TO_DATE('20240101', 'YYYYMMDD'), --시분초 생략하면 0시0분0초가 됨
TO_DATE('202401','YYYYMM'), --날짜 생략하면 1월 1일이 된다.
...
SELECT --조회가 되어야 nvl 사용 가능. null도 조회되어야 한다
     emp_nm,
...
SELECT
    --emp_nm
    --MAX(emp_nm) --이름이 없는 사람 조회하면 null이 나온다
    -- NVL(emp_nm, '존재안함') AS emp_nm
